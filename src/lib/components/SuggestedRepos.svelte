      ...
      {#each repos as repo}
        ...
        >
          <div class="flex justify-between items-start mb-2">
            <h4 class="font-bold text-zinc-200 group-hover:text-amber-300 transition-colors truncate pr-2">{repo.full_name}</h4>
            <div class="flex items-center gap-1 text-xs text-zinc-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-amber-400" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
              </svg>
              {formatNumber(repo.stargazers_count)}
            </div>
          </div>
          <p class="text-xs text-zinc-400 line-clamp-2 font-light leading-relaxed">
